...
import { Heart, MessageCircle, Share2, Send, Trophy, Dumbbell, Loader2 } from 'lucide-react';
          ...
          <div key={post.id} className="bg-card">
            ...
            {post.exercise_name && (
              <div className={cn("mx-4 mt-3 p-4 rounded-xl",
                post.is_pr ? "bg-gradient-to-r from-primary/15 to-accent/5 border border-primary/20" : "bg-secondary/40"
              )}>
                {post.is_pr && (
            ...
            <div className="flex items-center gap-1 px-2 py-2 mt-2">
              <button onClick={() => toggleLike(post.id)} className="flex items-center gap-1.5 px-3 py-2 rounded-full transition-colors">
